function out =  processBdotSignals(probe_id)

 % (1) Load oppositely-polarized signals
 
 load('marz_shot1_bdotprobes.mat');
 
 % probe_id = 'BADV120'; % Probe ID BADV<AzmuthalPosition><Top/Bottom><Polarity>
 probe_top = [probe_id, 'T']; % Top probe
 probe_bottom = [probe_id, 'B']; % Bottom Probe
 
 for ii = 1:length(all_probes)
     probes{ii} = all_probes{ii}(1:8); 
 end
 ...
 k_bottom = cal_fac(top_idx); % Ts^{-1} / V
 
 out = getInductiveSignal(time{top_idx},time{bottom_idx},t0,V{top_idx},V{bottom_idx},k_top,k_bottom,tlm);
 out.R = radial_dist(top_idx);
  out.id = probe_id;
 
 showInductiveSignal(out,probe_top,probe_bottom);
 
sgtitle([probe_id, ' ' num2str(radial_dist(top_idx)) 'mm'],'FontSize',24);
...
% magnetic field
    ...
    dtB = bdot_eff(idx1);
    out.B = cumtrapz(dtB) * mean(diff(t1)); % [T]
